...
def HP_Power(active, T_amb, T_ret, m_dot = 0.22, HE_eff = 0.8, T_sup = 273 + 53, P_max = 6, c = 4200):
    from math import e
    ...
    if active:
        ...
        if m_dot*c*(T_sup - T_ret)/(1000*COP) < P_max:
        
            return [m_dot*c*(T_sup - T_ret)/(1000*COP), HE_eff*m_dot*c*(T_sup - T_ret)]
        else:
            return [P_max, P_max*COP*1000]
    else:
        return [0,0]
...
def new_house_Temperature(T_0, Qdot_Losses, mc_T, Qdot_HP = 0, Qdot_TESS = 0, Qdot_SC = 0, Qdot_Boiler = 0, dt=0.25*3600):
    T_new = T_0 + dt*(Qdot_Losses + Qdot_HP + Qdot_TESS + Qdot_SC + Qdot_Boiler)/(mc_T)
...
def House_type_adjust(House_type = 'apartment'):
    if House_type == 'apartment':
        return [1, 1, 1, 1] #[window_adjust, walls_adjust, roof_adjust, volume_adjust]
        
    elif House_type == 'studio':
        return [0.5, 0.663, 0.467, 0.467,]
        
    elif House_type == 'stand_alone':
        return [1.5, 2.04, 1, 2]    

def Windows_losses_coefficient(glazing_type, h_air_window = 25, LWindow = 0.004, kWindow = 0.8, LCavity = 0.014, kAir = 0.0257, h_window_atm = 32):
    if glazing_type == 'single':
        return ((1/h_air_window) + (LWindow/kWindow) + (1/h_window_atm))**-1
    
    elif glazing_type == 'double':
        return ((1/h_air_window) + (LWindow/kWindow) + (LCavity/kAir) + (LWindow/kWindow) + (1/h_window_atm))**-1
        
    elif glazing_type == 'triple':
        return ((1/h_air_window) + (LWindow/kWindow) + (LCavity/kAir) + (LWindow/kWindow) + (LCavity/kAir) + (LWindow/kWindow) + (1/h_window_atm))**-1

def Roof_losses_coefficient(LRoof = 0.2, kRoof = 0.04, h_air_roof = 12, h_roof_atm = 38):    
    # LRoof = 0.03, 0.05-0.1, 0.05-0.1, 0.1-0.15, 0.15-0.2, 0.15-0.2, 0.2-0.3
    
    return ((1/h_air_roof) + (LRoof/kRoof) + (1/h_roof_atm))**-1
            
def Wall_losses_coefficient(Cavity_Filling = False, Cavity_type = 'air', Wall_Cover = False, LWall = 0.25, kWall = 0.14, LCavity = 0.05, kCavity = 0.0257,  LExternal = 0.1, kExternal = 0.035, h_air_wall = 0.9, h_wall_atm = 0.9):

    if Cavity_type == 'air':
        kCavity = 0.0257
    
    elif Cavity_type == 'EPS':
        kCavity = 0.035

    U_cavity = 0
    if Cavity_Filling:
        U_cavity = LCavity/kCavity
        # print('LCavity', LCavity)
        # print('kCavity', kCavity)
        # print('U_cavity ', U_cavity)

    U_cover = 0
    if Wall_Cover:
        U_cover = LExternal/kExternal
        
    return ((1/h_air_wall) + (LWall/kWall) + U_cavity + U_cover + (1/h_wall_atm))**-1
        

    # if Cavity_Filling and Wall_Cover:
    #     return ((1/h_air_wall) + (LWall/kWall) + (LCavity/kCavity) + (LExternal/kExternal) + (1/h_wall_atm))**-1
    
    # elif Cavity_Filling and not Wall_Cover:
    #     return ((1/h_air_wall) + (LWall/kWall) + (LCavity/kCavity) + (1/h_wall_atm))**-1
    
    # elif not Cavity_Filling and Wall_Cover:
    #     return ((1/h_air_wall) + (LWall/kWall) + (LExternal/kExternal) + (1/h_wall_atm))**-1
        
    # else:
    #     return ((1/h_air_wall) + (LWall/kWall) + (1/h_wall_atm))**-1
        
def House_Thermal_Losses(T_0_in, T_amb, U = 1, A_exposed = 119.6, C_stack = 0.015, C_wind = 0.0012, A_conditioned = 120, N_bedrooms = 1, c_air = 1012, density_air = 1.293, House_type = 'apartment', glazing_type = 'double', Cavity_Filling = False, Cavity_type = 'air', Wall_Cover = False, LRoof = 0.2, LCavity_wall = 0.05):
    ...
    windows_area  = (n1_window + n2_window + n3_window + n4_window) * htWindows * widWindows * window_adjust
    ...
    U_windows = Windows_losses_coefficient(glazing_type, h_air_window, LWindow, kWindow, LCavity, kAir, h_window_atm)
    
    if glazing_type == 'single':
        m_windows = windowDensity * windows_area * LWindow
    elif glazing_type == 'double':
        m_windows = windowDensity * windows_area * LWindow*2
    elif glazing_type == 'triple':
        m_windows = windowDensity * windows_area * LWindow*3   
    ...
    kCavity_wall= 0.0257
    LExternal_wall= 0.1
    kExternal_wall= 0.035
    
    
    walls_area = (2*(lenHouse + widHouse) * htHouse - windows_area) * walls_adjust
    U_wall = Wall_losses_coefficient(Cavity_Filling, Cavity_type, Wall_Cover, LWall, kWall, LCavity_wall, kCavity_wall, LExternal_wall, kExternal_wall, h_air_wall, h_wall_atm)
    ...
    roof_Area = 2 * (widHouse/(2*cos(pitRoof))*lenHouse) * roof_adjust
    U_roof = Roof_losses_coefficient(LRoof, kRoof, h_air_roof, h_roof_atm)
    # U_roof = ((1/h_air_roof) + (LRoof/kRoof) + (1/h_roof_atm))**-1
    m_roof = roofDensity * roof_Area * LRoof
    
    
    m_air = airDensity * lenHouse * widHouse * htHouse * volume_adjust
...
def Thermal_model(Thermal_Components, T_in_0, T4, T_TESS_0, T_amb, T_set, G, Qdot_TESS, Qdot_HP, n_modules = 5, PVT_active = False, PVT_TESS_charge = False, T_tank_PVT = 50, T_PVT_layers = [25, 25, 25, 25], TESS_min_op_T = 55 + 273, TESS_max_op_T = 90 + 273, Network_losses = False, m = 4000, T_sup = 50 + 273, T_TESS_max = 273 + 95, m_dot = 0.22, c_f = 4200, dt = 60*15):
    from MCES_library import House_Thermal_Losses, pipe_losses, new_house_Temperature, Qdot_PVT
    
    fluid_registry = [[], [], [], [], [T4 - 273]]
    T_registry = [[], [], [], [], []]


    [Qdot_Losses_i, mc_T] = House_Thermal_Losses(T_in_0[0], T_amb)     
    Q_dot_network = -(mc_T*(T_in_0[0] - T_in_0[1])/dt - Qdot_Losses_i) #  
    T0 = T4 + Q_dot_network/(m_dot*c_f)    
    T_registry[0].append(T0)   
    
    if Network_losses:
        Q_dot_network += -pipe_losses(T0, T_in_0[-1], m_dot)
    else:
        Q_dot_network += 0
    ...
    if Thermal_Components[0]:  
        ...
        [Q_dot_SC, T_PVT_layers, T_tank_PVT, T2, Qdot_PVT2Network, Qdot_PVT2TESS] = Qdot_PVT(PVT_active, PVT_TESS_charge, int(T_amb - 273), G/3600, 1, T1-273, T_PVT_layers[0], T_PVT_layers[1], T_PVT_layers[2], T_PVT_layers[3], T_tank_PVT, T_TESS_0-273, n_HE = 1, m_f_dot = 0.029085, t_end = int(15*60), n_modules = n_modules)
        T2 += 273     
    ...
    if Thermal_Components[1]:
        ...
        T3 = T2 + (Q_dot_network + Qdot_PVT2Network + Qdot_TESS)/(m_dot*c_f) 
    ...
    if Thermal_Components[2]:    
        ...
        T4 += (Q_dot_network + Qdot_PVT2Network + Qdot_TESS + Qdot_HP)/(m_dot*c_f)  
    ...
    T_in = new_house_Temperature(T_in_0[0], Qdot_Losses_i, mc_T, Qdot_HP = Qdot_HP, Qdot_TESS = Qdot_TESS, Qdot_SC = Qdot_PVT2Network)
    
    
    return [T1, T2, T3, T4, T_in]    